...
Each run is independent of other runs
...
---
# Promises
### Lazy evaluation is powered by Promises : a data structure 
...
---
# Missing arguments
you can use `missing()` to see if the argument's value comes from the user or defualt
```{r eval=FALSE}
h06 <- function(x = 10) {
  list(missing(x), x)
}
...
- Knowing the name of the function that underlies a special form is useful for getting documentation: 
  - ?( is a syntax error; ?`(` will give you the documentation for parentheses.
